...
const INCOME_SOURCES = ["Employment", "Self-employment", "Section 8 Voucher", "VASH Voucher", "SSI/SSDI", "Retirement"];
...
// ---------------------------------------------------------------------------
// Core seed logic (shared between server action and standalone seed script)
// ---------------------------------------------------------------------------

async function generateDemoData(organizationId: string) {
  // ------ Properties ------
    ...
    Array.from({ length: 20 }, () => {
      ...
      return prisma.applicant.create({
        data: {
          ...
          sourceOfIncome: hasVoucher ? pick(["Section 8 Voucher", "VASH Voucher"]) : pick(["Employment", "Self-employment", "SSI/SSDI"]),
        },
      });
    })
  );

  // ------ Applications (one per applicant, spread across properties) ------
  ...
  // ------ Screening Records (~4 per application) ------
    ...
    for (const config of recordConfigs) {
      const record = await prisma.screeningRecord.create({
        data: {
          ...
          rawData: JSON.parse(JSON.stringify(config.rawData)),
          normalizedData: JSON.parse(JSON.stringify(config.normalizedData)),
  ...
  for (let i = 0; i < decisionApplications.length; i++) {
    ...
    if (outcome === "DENIED" || outcome === "CONDITIONAL" || outcome === "PENDING_REVIEW") {
      ...
      if (outcome === "DENIED") {
        const denyRule = policyRules.find((r) => r.criterionType === "EVICTION_HISTORY" || r.criterionType === "CRIMINAL_HISTORY");
        reasonCodeData.push({
          code: "CR-001",
          category: "Credit",
    ...
    await prisma.notice.create({
      data: {
        ...
        content: JSON.parse(JSON.stringify({
          applicantName: `${(await prisma.applicant.findUnique({ where: { id: app.applicantId } }))?.firstName} ${(await prisma.applicant.findUnique({ where: { id: app.applicantId } }))?.lastName}`,
          propertyName: properties[Number(noticeApps.indexOf(app)) % properties.length].name,
  ...
  const jurisdictionRules = [
    // Federal rules
    {
      ...
      ruleData: JSON.parse(JSON.stringify({
...
export async function seedDemoData(): Promise<ActionResult> {
  try {
    ...
    await prisma.jurisdictionRule.deleteMany({
      where: { jurisdiction: { organizationId: org.id } },
    });
    await prisma.jurisdiction.deleteMany({ where: { organizationId: org.id } });
    await prisma.driftAlert.deleteMany({ where: { organizationId: org.id } });
    await prisma.fairnessMetric.deleteMany({ where: { organizationId: org.id } });

    // Notices, challenges, documents, accommodations depend on applications
    const appIds = (
    ...
    if (appIds.length > 0) {
      ...
      // Decisions (and their reason codes via cascade, humanReview, override)
      const decisionIds = (
    ...
    await prisma.applicant.deleteMany({ where: { organizationId: org.id } });

    // Policy rules depend on screening policies
    const policyIds = (
    ...
    await prisma.screeningPolicy.deleteMany({ where: { organizationId: org.id } });
    await prisma.property.deleteMany({ where: { organizationId: org.id } });

    // Generate fresh demo data
    await generateDemoData(org.id);
